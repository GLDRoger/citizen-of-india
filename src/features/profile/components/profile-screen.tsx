...
function AssetRow({ asset }: { asset: ReturnType<typeof getOwnedAssets>[number] }) {
  if (asset.type === "business") {
    return <div className="flex min-h-20 items-center gap-4 border-t border-paper-line py-3"><span className="grid size-10 place-items-center rounded-[4px] bg-green-tint text-green-deep"><BriefcaseBusiness aria-hidden className="size-5" /></span><div className="min-w-0 flex-1"><strong className="block truncate text-sm text-ink">{asset.attrs.name}</strong><span className="text-xs capitalize text-ink-mute">{asset.attrs.entityType} · {formatCurrency(asset.attrs.turnoverFY25)} FY25 turnover</span></div><VerificationBadge verification={asset.verification} /></div>;
  }
  if (asset.type === "vehicle") {
    return <div className="flex min-h-20 items-center gap-4 border-t border-paper-line py-3"><span className="grid size-10 place-items-center rounded-[4px] bg-paper-line text-ink"><CarFront aria-hidden className="size-5" /></span><div className="min-w-0 flex-1"><strong className="block truncate text-sm text-ink">{asset.attrs.make} {asset.attrs.model}</strong><span className="text-xs text-ink-mute">{maskIdentifier(asset.attrs.regNumber)}</span></div><VerificationBadge verification={asset.verification} /></div>;
  }
  if (asset.type === "property") {
    return <div className="flex min-h-20 items-center gap-4 border-t border-paper-line py-3"><span className="grid size-10 place-items-center rounded-[4px] bg-brick-tint text-brick"><Home aria-hidden className="size-5" /></span><div className="min-w-0 flex-1"><strong className="block truncate text-sm capitalize text-ink">{asset.attrs.kind}</strong><span className="text-xs text-ink-mute">{asset.attrs.authority} · {formatCurrency(asset.attrs.estimatedValue)}</span></div><VerificationBadge verification={asset.verification} /></div>;
  }
  return null;
}
    ...
    <Page className="grid gap-10">
      ...
      <div className="grid gap-8 lg:grid-cols-[1.25fr_0.75fr]">
        <div className="grid gap-9">
          <section className="grid gap-5"><SectionHeader eyebrow={`${relationships.length}`} title={t("relationships")} /><div className="grid gap-3 sm:grid-cols-2">{relationships.map(({ person, relationship }) => <article className="flex min-h-28 items-center gap-4 rounded-[8px] border border-paper-line bg-paper-shade p-4" key={person.id}><span className="grid size-11 place-items-center rounded-[4px] bg-paper-line text-ink"><UserRound aria-hidden className="size-5" /></span><div className="min-w-0"><strong className="block truncate text-sm text-ink">{person.attrs.name}</strong><span className="text-xs capitalize text-ink-mute">{relationship}{person.attrs.deceasedOn ? ` · died ${formatDate(person.attrs.deceasedOn)}` : ""}</span></div></article>)}</div></section>

          <section className="grid gap-5"><SectionHeader title={t("workAndBusiness")} />{employment ? <div className="flex min-h-24 items-center gap-4 border-y border-paper-line py-4"><span className="grid size-11 place-items-center rounded-[4px] bg-green-tint text-green-deep"><Building2 aria-hidden className="size-5" /></span><div className="min-w-0 flex-1"><strong className="block truncate text-sm text-ink">{employment.attrs.employer}</strong><span className="text-xs text-ink-mute">{employment.attrs.designation} · {employment.attrs.location}</span></div><VerificationBadge verification={employment.verification} /></div> : null}{assets.filter((asset) => asset.type === "business").map((asset) => <AssetRow asset={asset} key={asset.id} />)}</section>

          <section className="grid gap-5"><SectionHeader title={t("propertyAndVehicles")} />{assets.filter((asset) => asset.type !== "business").length ? <div>{assets.filter((asset) => asset.type !== "business").map((asset) => <AssetRow asset={asset} key={asset.id} />)}</div> : <p className="border-y border-paper-line py-6 text-sm text-ink-mute">No property or vehicles connected to this citizen.</p>}</section>
        </div>
        <div className="grid content-start gap-5">
          <DelegationPanel personId={personId} />
          <div className="grid gap-4 rounded-[8px] border border-paper-line bg-paper-shade p-5"><div className="flex items-center gap-3"><ShieldCheck aria-hidden className="size-5 text-green-deep" /><strong className="text-sm text-ink">Record health</strong></div><div className="grid grid-cols-2 gap-4"><div><span className="block text-xs text-ink-mute">{t("verified")}</span><strong className="font-display text-2xl text-ink">{profile.verifiedDocumentCount}/{profile.documentCount}</strong></div><div><span className="block text-xs text-ink-mute">{t("relationships")}</span><strong className="font-display text-2xl text-ink">{relationships.length}</strong></div></div><p className="flex items-center gap-2 text-xs text-ink-mute"><UsersRound aria-hidden className="size-4" /> Earlier family relationships stay in your history.</p><p className="flex items-center gap-2 text-xs text-ink-mute"><Clock3 aria-hidden className="size-4" /> Prior versions remain available when records change.</p></div>
        </div>
      </div>
    </Page>
  );
