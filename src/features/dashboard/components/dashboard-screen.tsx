...
import { AlertTriangle, ArrowRight, Bell, CalendarDays, ExternalLink, IndianRupee, MessageSquareText, SearchCheck } from "lucide-react";
...
function DocumentsRail({ personId }: { personId: string }) {
  ...
  return (
    <aside className="grid content-start gap-5 rounded-[8px] border border-paper-line bg-paper-shade p-5 lg:sticky lg:top-24">
      <div className="flex items-center justify-between gap-3"><h2 className="font-display text-2xl font-semibold text-ink">{t("documents")}</h2><span className="font-display text-2xl font-semibold text-green-deep">{documents.length}</span></div>
    ...
    <Page className="grid gap-10 lg:gap-14">
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_19rem] lg:items-start">
        <div className="grid gap-10">
          ...
          <section className="grid gap-5"><SectionHeader title={t("recentActivity")} />{events.length ? <ol className="border-y border-paper-line">{events.slice(0, 6).map((event) => <li className="grid grid-cols-[auto_minmax(0,1fr)] gap-4 border-b border-paper-line py-4 last:border-b-0" key={event.id}><span className="mt-1.5 size-2 rounded-[4px] bg-green-deep" /><div><strong className="block text-sm text-ink">{event.label}</strong><span className="text-xs text-ink-mute">{new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(event.occurredAt))}</span></div></li>)}</ol> : <EmptyState title={t("noActivity")} />}</section>
        </div>
