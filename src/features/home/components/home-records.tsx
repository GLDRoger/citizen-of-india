...
function RecordsShortcut({ personId }: { personId: string }) {
  const { t } = useI18n();
  const graph = useCitizenStore((state) => state.graph);
  const count = getDocuments(graph, personId).length;
  return (
    <aside className="grid content-start gap-3 border-y border-paper-line py-5">
      <h2 className="font-display text-2xl font-semibold text-ink">{t("you")}</h2>
      <p className="text-sm leading-6 text-ink-mute">{t("homeRecordsShortcutBody", { count })}</p>
      <Link className="flex min-h-11 items-center justify-between gap-3 text-sm font-bold text-indigo-deep underline decoration-indigo-deep/25 underline-offset-4" href="/you">{t("fullProfile")}<ArrowRight aria-hidden className="size-4" /></Link>
    </aside>
  );
}
...
export function HomeRecords({ personId }: { personId: string }) {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start">
      ...
      <RecordsShortcut personId={personId} />
    </div>
  );
}
