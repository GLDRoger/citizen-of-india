...
function DocumentCard({ document, personId }: { document: ReturnType<typeof getDocuments>[number]; personId: string }) {
  ...
  return (
    <article className="grid scroll-mt-24 content-start gap-5 rounded-[3px] border border-paper-line bg-panel p-5 target:border-ink" id={document.id}>
      ...
      {document.verification.state === "mismatch" && person ? <div className="grid gap-2 rounded-[2px] bg-brick-tint px-3 py-3 text-xs leading-5 text-brick"><p className="flex gap-2"><AlertTriangle aria-hidden className="mt-0.5 size-3.5 shrink-0" />{t("panMismatchExplanation", { aadhaarName: person.attrs.name, panName: document.attrs.holderName })}</p></div> : null}
      <div className="mt-auto grid gap-2">
        {document.verification.state === "mismatch" && !actorId ? <LinkButton href="/workflows/record-correction"><ShieldCheck aria-hidden className="size-4" />{t("resolveMismatch")}</LinkButton> : null}
        {error ? <p className="text-sm text-brick" role="alert">{t("sharedSaveError")}</p> : null}
        ...
        <details className="group border-t border-paper-line pt-2">
          ...
          <div className="grid gap-2 pb-2 text-xs leading-5 text-ink-mute"><ProvenanceLine verification={document.verification} /><span>{t("documentUseReason")}</span></div>
        </details>
      </div>
    </article>
  );
  ...
  return (
    <Page className="grid gap-6">
      ...
      <label className="relative block max-w-xl">
        ...
        <input className="h-13 w-full rounded-[3px] border border-paper-line bg-panel pl-11 pr-4 text-sm outline-none transition focus:border-indigo-deep focus:ring-4 focus:ring-indigo-tint" onChange={(event) => setQuery(event.target.value)} placeholder={t("searchDocuments")} type="search" value={query} />
      </label>
      {visible.length ? (
