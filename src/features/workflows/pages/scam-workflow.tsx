...
export function ScamWorkflow() {
  ...
  const content = complete ? (
    ...
    <div id="report-draft"><StepCard eyebrow={t("scamResultEyebrow")} title={analysis.summary} body={t("scamResultBody")}><div className="flex flex-wrap items-center gap-2"><StatusPill label={verdictKey ? t(verdictKey) : analysis.verdict} tone={analysis.verdict === "suspicious" ? "warning" : "info"} /><StatusPill label={t("confidenceLabel", { confidence: confidenceKey ? t(confidenceKey) : analysis.confidence })} tone="neutral" /><SimulatedChip authority={analysis.authority} /></div><div className="grid gap-2">{analysis.signals.map((signal) => <p className="flex gap-2 text-xs leading-5 text-ink-mute" key={signal}><AlertTriangle aria-hidden className="mt-0.5 size-3.5 shrink-0 text-brick" />{signal}</p>)}</div><div className="grid gap-2 rounded-[8px] bg-green-tint p-4"><strong className="flex items-center gap-2 text-sm text-ink"><ShieldAlert aria-hidden className="size-4 text-green-deep" />{t("scamSafeActions")}</strong>{analysis.nextActions.map((action) => <p className="flex gap-2 text-xs leading-5 text-ink-mute" key={action}><Check aria-hidden className="mt-0.5 size-3.5 shrink-0 text-green-deep" />{action}</p>)}</div><div className="flex flex-col gap-2 sm:flex-row"><Button loading={loading} onClick={() => void startReport()}>{t("scamCreateDraft")}</Button><Button onClick={() => setAnalysis(null)} variant="secondary">{t("scamCheckAnother")}</Button></div></StepCard></div>
  );
