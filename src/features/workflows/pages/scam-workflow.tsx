...
import { getNodeByType, getNotices } from "@/features/graph/selectors";
...
export function ScamWorkflow() {
  ...
  const exampleMessage = getNodeByType(graph, "ntc:scam-pan", "notice")?.attrs.body ?? "";
  ...
  const startReport = async () => {
    ...
    try {
      ...
      const mutations: GraphMutation[] = [
        { type: "addNode", node: { id: reportId, type: "application", attrs: { title: t("scamReportTitle"), authority: response.authority, status: "draft", createdOn: "2026-08-24", kind: "cybercrime", participants: [personId], reference: response.data.acknowledgement, note: t("scamReportNote") }, verification: { source: "Self", state: "self-declared", asOf: "2026-08-24" } } },
  ...
  const content = complete ? (
    <CompletionCard title={t("scamCompleteTitle")} body={t("scamCompleteBody")}><LinkButton href="/#money" variant="inverse">{t("scamViewDraft")} <ArrowRight aria-hidden className="size-4" /></LinkButton></CompletionCard>
  ) : !analysis ? (
    <StepCard eyebrow={t("scamPasteEyebrow")} title={t("scamPasteTitle")} body={t("scamPasteBody")}><button className="min-h-10 self-start rounded-[4px] border border-paper-line bg-paper px-3 font-display text-xs font-semibold text-ink transition-colors hover:border-green-deep/40" onClick={() => setMessage(exampleMessage)} type="button">{t("scamTryExample")}</button><label className="grid gap-2"><span className="text-xs font-bold text-ink">{t("scamMessageLabel")}</span><textarea className="min-h-40 resize-y rounded-[8px] border border-paper-line bg-paper p-4 text-sm leading-6 text-ink outline-none focus:border-green-deep focus:ring-4 focus:ring-green-deep/10" maxLength={2000} onChange={(event) => setMessage(event.target.value)} placeholder={t("scamMessagePlaceholder")} value={message} /></label><p className="flex gap-2 text-xs text-ink-mute"><Link2Off aria-hidden className="size-4 shrink-0 text-brick" />{t("scamSafetyNote")}</p><Button disabled={message.trim().length < 5} loading={loading} onClick={() => void analyze()}><MessageSquareWarning aria-hidden className="size-4" />{t("scamAnalyzeAction")}</Button></StepCard>
  ) : (
    <div id="report-draft"><StepCard eyebrow={t("scamResultEyebrow")} title={analysis.summary} body={t("scamResultBody")}><div className="flex flex-wrap items-center gap-2"><StatusPill label={verdictKey ? t(verdictKey) : analysis.verdict} tone={analysis.verdict === "suspicious" ? "warning" : "info"} /><StatusPill label={t("confidenceLabel", { confidence: confidenceKey ? t(confidenceKey) : analysis.confidence })} tone="neutral" /><SimulatedChip authority={analysis.authority} /></div><ContrastLine>{t("contrastScamHeader")}</ContrastLine><div className="grid gap-2">{analysis.signals.map((signal) => <p className="flex gap-2 text-xs leading-5 text-ink-mute" key={signal}><AlertTriangle aria-hidden className="mt-0.5 size-3.5 shrink-0 text-brick" />{signal}</p>)}</div><div className="grid gap-2 rounded-[8px] bg-green-tint p-4"><strong className="flex items-center gap-2 text-sm text-ink"><ShieldAlert aria-hidden className="size-4 text-green-deep" />{t("scamSafeActions")}</strong>{analysis.nextActions.map((action) => <p className="flex gap-2 text-xs leading-5 text-ink-mute" key={action}><Check aria-hidden className="mt-0.5 size-3.5 shrink-0 text-green-deep" />{action}</p>)}</div><div className="flex flex-col gap-2 sm:flex-row"><Button loading={loading} onClick={() => void startReport()}>{t("scamCreateDraft")}</Button><Button onClick={() => setAnalysis(null)} variant="secondary">{t("scamCheckAnother")}</Button></div></StepCard></div>
  );

  return <ProcedureShell authority={t("scamWorkflowAuthority")} complete={complete} currentStep={currentStep} description={t("scamPromise")} procedureId="scam-check" steps={steps} title={t("scamService")}>{error ? <p className="mb-3 rounded-xl bg-brick-tint p-3 text-sm font-semibold text-brick" role="alert">{error}</p> : null}{content}</ProcedureShell>;
}
