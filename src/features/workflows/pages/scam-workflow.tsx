...
export function ScamWorkflow() {
  ...
  const content = complete ? (
    <CompletionCard title="A safe report draft is ready." body="Citizen preserved the suspicious message and warning signs in a local draft. Nothing was sent to a live cybercrime portal."><LinkButton href="/activity" variant="inverse">View draft <ArrowRight aria-hidden className="size-4" /></LinkButton></CompletionCard>
  ) : !analysis ? (
  ...
  return <ProcedureShell authority="Citizen safety analysis + simulated cybercrime portal" complete={complete} currentStep={currentStep} description="Inspect a message without opening links, compare it with known interactions, and prepare a safe report draft." steps={steps} title="Scam check">{error ? <p className="mb-3 rounded-xl bg-danger-soft p-3 text-sm font-semibold text-danger" role="alert">{error}</p> : null}{content}</ProcedureShell>;
}
