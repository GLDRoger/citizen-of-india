...
export function MarriageWorkflow() {
  ...
  const content = complete ? (
    ...
    <StepCard eyebrow="Two citizens, one application" title={personId === arjun.id ? "Invite Priya to register together" : "Arjun starts this shared application"} body="Each person keeps control of their own consent and verified documents."><div className="flex items-center gap-4 rounded-[18px] bg-surface-strong p-4"><span className="grid size-11 place-items-center rounded-full bg-ink text-canvas"><Heart aria-hidden className="size-5" /></span><div className="flex-1"><strong className="block text-sm">Arjun Sharma + Priya Patel</strong><span className="text-xs text-ink-muted">Bengaluru · Ahmedabad</span></div><SimulatedChip authority="Citizen invite relay" /></div>{personId === arjun.id ? <Button onClick={invite}>Invite Priya <ArrowRight aria-hidden className="size-4" /></Button> : <Button onClick={() => switchTo(arjun.id)} variant="secondary">Switch to Arjun</Button>}</StepCard>
  ) : currentStep === 1 ? (
    <StepCard eyebrow="Consent required" title={personId === priya.id ? "Priya, do you consent?" : "Waiting for Priya"} body="The invitation names Arjun, the purpose, documents requested and the exact graph change that will happen after registration."><div className="grid gap-2 rounded-[18px] bg-action-soft p-4 text-xs text-ink-muted"><span className="flex gap-2"><Check aria-hidden className="size-4 text-action" />Identity verification</span><span className="flex gap-2"><Check aria-hidden className="size-4 text-action" />Reuse of verified identity documents only</span><span className="flex gap-2"><Check aria-hidden className="size-4 text-action" />New spouse relationship after submission</span></div>{personId === priya.id ? <Button onClick={consent}><UserRoundCheck aria-hidden className="size-4" />I consent</Button> : <Button onClick={() => switchTo(priya.id)} variant="secondary"><UsersRound aria-hidden className="size-4" />Switch to Priya’s login</Button>}</StepCard>
  ) : currentStep === 2 ? (
