...
export function MarriageWorkflow() {
  ...
  const content = complete ? (
    ...
    <StepCard eyebrow={t("marriageSharedEyebrow")} title={personId === arjun.id ? t("marriageInviteTitle") : t("marriageStartWithArjun")} body={t("marriageSharedBody")}><div className="flex items-center gap-4 rounded-[8px] bg-paper-line p-4"><span className="grid size-11 place-items-center rounded-[4px] bg-ink text-paper"><Heart aria-hidden className="size-5" /></span><div className="flex-1"><strong className="block text-sm">Arjun Sharma + Priya Patel</strong><span className="text-xs text-ink-mute">Bengaluru · Ahmedabad</span></div><SimulatedChip authority="Citizen invite relay" /></div>{personId === arjun.id ? <Button onClick={invite}>{t("marriageInvitePriya")} <ArrowRight aria-hidden className="size-4" /></Button> : <Button onClick={() => switchTo(arjun.id)} variant="secondary">{t("switchArjun")}</Button>}</StepCard>
  ) : currentStep === 1 ? (
