...
export function MarriageWorkflow() {
  ...
  const content = complete ? (
    <CompletionCard title={t("marriageCompleteTitle")} body={t("marriageCompleteBody")}><div className="flex flex-wrap gap-2"><LinkButton href="/you" variant="inverse">{t("marriageViewRelationship")} <ArrowRight aria-hidden className="size-4" /></LinkButton><LinkButton href="/documents" variant="secondary">{t("marriageOpenCertificate")}</LinkButton></div></CompletionCard>
  ) : currentStep === 0 ? (
    <StepCard eyebrow={t("marriageSharedEyebrow")} title={personId === arjun.id ? t("marriageInviteTitle") : t("marriageStartWithArjun")} body={t("marriageSharedBody")}><ContrastLine>{t("contrastMarriageInvite")}</ContrastLine><div className="flex items-center gap-4 rounded-[8px] bg-paper-line p-4"><span className="grid size-11 place-items-center rounded-[4px] bg-ink text-paper"><Heart aria-hidden className="size-5" /></span><div className="flex-1"><strong className="block text-sm">Arjun Sharma + Priya Patel</strong><span className="text-xs text-ink-mute">Bengaluru · Ahmedabad</span></div><SimulatedChip authority="Citizen invite relay" /></div>{personId === arjun.id ? <Button onClick={invite}>{t("marriageInvitePriya")} <ArrowRight aria-hidden className="size-4" /></Button> : <Button onClick={() => switchTo(arjun.id)} variant="secondary">{t("switchArjun")}</Button>}</StepCard>
  ) : currentStep === 1 ? (
    ...
    <StepCard eyebrow={t("marriageRecordsEyebrow")} title={t("marriageRecordsTitle")} body={t("marriageRecordsBody", { count: reusedDocumentCount })}><div className="grid gap-4 sm:grid-cols-2">{[{ name: "Arjun", docs: arjunDocs }, { name: "Priya", docs: priyaDocs }].map((group) => <div className="grid gap-2 rounded-[8px] bg-paper-line p-4" key={group.name}><strong className="text-sm">{group.name}</strong>{group.docs.map((document) => { const kindKey = getDocumentKindMessageKey(document.attrs.kind); return <span className="flex items-center justify-between gap-2 text-xs text-ink-mute" key={document.id}><span className="flex items-center gap-2"><FileStack aria-hidden className="size-3.5" />{kindKey ? t(kindKey) : document.attrs.kind}</span><VerificationBadge verification={document.verification} /></span>; })}</div>)}</div><div className="grid gap-2"><p className="text-xs font-bold text-ink">{t("marriageWitnesses")}</p><div className="grid gap-2 sm:grid-cols-2">{witnessCandidates.map((witness) => { const selected = selectedWitnesses.includes(witness.id); return <button aria-pressed={selected} className={`flex min-h-14 items-center justify-between rounded-[8px] border px-4 text-left text-sm font-bold transition ${selected ? "border-green-deep bg-green-tint text-green-deep" : "border-paper-line bg-paper-shade"}`} key={witness.id} onClick={() => toggleWitness(witness.id)}><span>{witness.attrs.name}</span>{selected ? <Check aria-hidden className="size-4" /> : null}</button>; })}</div></div><Button disabled={selectedWitnesses.length === 0} onClick={reuseDocuments}>{t("marriageUseDocuments")} <ArrowRight aria-hidden className="size-4" /></Button></StepCard>
  ) : currentStep === 3 ? (
