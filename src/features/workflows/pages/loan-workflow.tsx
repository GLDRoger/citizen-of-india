  ...
  const content = complete ? (
    ...
    <div className="grid gap-7">
      <article className="grid gap-6 rounded-[22px] border border-action/30 bg-action-soft p-6 sm:grid-cols-[minmax(0,1fr)_16rem] sm:items-end sm:p-8">
        <div className="grid gap-4"><div className="flex flex-wrap items-center gap-2"><StatusPill label={t("potentiallyEligible")} tone="warning" /><SimulatedChip authority="SIDBI / member banks" /></div><div className="grid gap-2"><p className="eyebrow">{t("recommendation")}</p><h2 className="font-display text-4xl font-semibold leading-none tracking-[-0.04em] text-ink">{selectedOption.name}</h2><p className="max-w-2xl text-sm leading-6 text-ink-muted">{risk || localRisk}</p></div></div>
        <div className="grid gap-3"><div className="flex items-end justify-between gap-3"><span className="text-xs font-bold text-ink-muted">{formatCurrency(desiredAmount)}</span><strong className="font-display text-2xl text-ink">{affordability}%</strong></div><div className="h-2 overflow-hidden rounded-full bg-surface"><div className="h-full rounded-full bg-action" style={{ width: `${affordability}%` }} /></div><span className="text-xs text-ink-muted">{business.attrs.name} · {formatCurrency(business.attrs.turnoverFY25)}</span></div>
      </article>

      <section className="grid gap-4"><h2 className="font-display text-3xl font-semibold tracking-[-0.035em] text-ink">{steps[1].title}</h2><div className="grid gap-4 md:grid-cols-2">{loanOptions.map((option) => <button aria-pressed={selected === option.id} className={`grid min-h-60 content-between gap-8 rounded-[20px] border p-5 text-left transition-colors ${selected === option.id ? "border-action bg-action-soft" : "border-line bg-surface hover:border-action/35"}`} key={option.id} onClick={() => { setSelected(option.id); setRisk(""); }}><div className="flex items-start justify-between gap-3"><Landmark aria-hidden className="size-5 text-action" /><StatusPill label={option.rate} tone={selected === option.id ? "info" : "neutral"} /></div><div className="grid gap-2"><strong className="font-display text-2xl font-semibold leading-tight text-ink">{option.name}</strong><span className="text-sm font-bold text-action-strong">{option.amount} · {option.term}</span><p className="text-xs leading-5 text-ink-muted">{option.note}</p></div></button>)}</div></section>
