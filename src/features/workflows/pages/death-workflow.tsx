...
export function DeathWorkflow() {
  ...
  const content = complete ? (
    ...
    <StepCard eyebrow={t("deathRegistryAuthority")} title={t("deathReviewRegistration")} body={t("deathReviewRegistrationBody")}><div className="grid gap-3 rounded-[8px] bg-paper-line p-4 text-sm"><span className="flex justify-between gap-3"><span className="text-ink-mute">{t("fieldName")}</span><strong>{rajesh.attrs.name}</strong></span><span className="flex justify-between gap-3"><span className="text-ink-mute">{t("fieldDate")}</span><strong>{formatDate("2026-08-24", language)}</strong></span><span className="flex justify-between gap-3"><span className="text-ink-mute">{t("fieldReporter")}</span><strong>{getPerson(graph, personId)?.attrs.name}</strong></span></div><ProcessLines carry={t("deathRegistrationCarry")} office={t("deathRegistrationOffice")} timeline={t("deathRegistrationTime")} /><Button loading={loading} onClick={() => void register()}>{t("deathRegisterAction")} <ArrowRight aria-hidden className="size-4" /></Button></StepCard>
  ) : currentStep === 2 ? (
