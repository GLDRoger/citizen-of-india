...
function ProcessLines({ carry, office, timeline }: { carry: string; office: string; timeline: string }) {
  const { t } = useI18n();
  return <dl className="grid border-y border-paper-line text-xs leading-5 text-ink-mute"><div className="grid grid-cols-[6rem_minmax(0,1fr)] gap-3 border-b border-paper-line py-2"><dt className="font-bold text-ink">{t("processOffice")}</dt><dd>{office}</dd></div><div className="grid grid-cols-[6rem_minmax(0,1fr)] gap-3 border-b border-paper-line py-2"><dt className="font-bold text-ink">{t("processCarry")}</dt><dd>{carry}</dd></div><div className="grid grid-cols-[6rem_minmax(0,1fr)] gap-3 py-2"><dt className="font-bold text-ink">{t("processTime")}</dt><dd>{timeline}</dd></div></dl>;
}
...
export function DeathWorkflow() {
  ...
  const collectHeirConsent = () => run(async () => {
    ...
    mutations.push(
      { type: "addNode", node: { id: "ntc:kavita-consent-received", type: "notice", attrs: { channel: "sms", sender: "CITIZEN-CONSENT", receivedOn: "2026-08-25", subject: "Kavita's legal-heir consent received", body: "The simulated consent relay recorded Kavita Verma's approval for the Rajesh Sharma legal-heir workflow.", legitimacy: "legitimate", relatedTo: "app:death-rajesh" }, verification: sourceVerification() } },
      { type: "addEdge", edge: { id: "e:arjun-subject-kavita-consent-notice", type: "subjectOf", from: "person:arjun", to: "ntc:kavita-consent-received", attrs: { read: false }, validFrom: "2026-08-25", status: "active", verification: sourceVerification() } },
    );
  ...
  const content = complete ? (
    ...
    <StepCard eyebrow={t("deathVerifiedFamilyRecords")} title={relationshipTitle} body={`${relationshipBody} ${t("deathConfirmBeforeChange")}`}><div className="grid gap-4 rounded-[3px] bg-paper-line p-4 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center"><span className="grid size-12 place-items-center rounded-[50%] border border-paper-line bg-panel font-display text-sm font-bold text-ink">{getInitials(rajesh.attrs.name)}</span><div><strong className="block text-ink">{rajesh.attrs.name}</strong><span className="text-xs text-ink-mute">{t("deathPensionerLocation")}</span></div><VerificationBadge verification={rajesh.verification} /></div><Button onClick={identify}>{t("deathConfirmRajesh")} <ArrowRight aria-hidden className="size-4" /></Button></StepCard>
  ) : currentStep === 1 ? (
    <StepCard eyebrow={t("deathRegistryAuthority")} title={t("deathReviewRegistration")} body={t("deathReviewRegistrationBody")}><div className="grid gap-3 rounded-[3px] bg-paper-line p-4 text-sm"><span className="flex justify-between gap-3"><span className="text-ink-mute">{t("fieldName")}</span><strong>{rajesh.attrs.name}</strong></span><span className="flex justify-between gap-3"><span className="text-ink-mute">{t("fieldDate")}</span><strong>{formatDate("2026-08-24", language)}</strong></span><span className="flex justify-between gap-3"><span className="text-ink-mute">{t("fieldReporter")}</span><strong>{getPerson(graph, personId)?.attrs.name}</strong></span></div><ProcessLines carry={t("deathRegistrationCarry")} office={t("deathRegistrationOffice")} timeline={t("deathRegistrationTime")} /><Button loading={loading} onClick={() => void register()}>{t("deathRegisterAction")} <ArrowRight aria-hidden className="size-4" /></Button></StepCard>
  ) : currentStep === 2 ? (
    <StepCard eyebrow={application?.attrs.reference} title={t("deathCertificateTitle")} body={t("deathCertificateBody")}><div className="flex items-center gap-4 rounded-[3px] bg-green-tint p-4"><FileCheck2 aria-hidden className="size-7 text-green-deep" /><div><strong className="block text-ink">{t("documentDeathCertificate")}</strong><span className="text-xs text-ink-mute">{t("deathCertificateVerified")}</span></div></div><ProcessLines carry={t("deathCertificateCarry")} office={t("deathCertificateOffice")} timeline={t("deathCertificateTime")} /><Button loading={loading} onClick={() => void issueCertificate()}>{t("deathIssueCertificate")} <ArrowRight aria-hidden className="size-4" /></Button></StepCard>
  ) : currentStep === 3 ? (
    <StepCard eyebrow="EPFO" title={t("deathClaimsTitle")} body={t("deathClaimsBody")}><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-[3px] bg-paper-line p-4"><Landmark aria-hidden className="mb-5 size-5 text-green-deep" /><strong className="block text-sm">{t("deathFamilyPension")}</strong><span className="text-xs text-ink-mute">≈ {t("monthlyAmount", { amount: "₹4,100" })}</span></div><div className="rounded-[3px] bg-paper-line p-4"><HeartHandshake aria-hidden className="mb-5 size-5 text-green-deep" /><strong className="block text-sm">{t("deathEpfClaim")}</strong><span className="text-xs text-ink-mute">{formatCurrency(rajesh.attrs.epf?.balance ?? 0)}</span></div></div><ProcessLines carry={t("deathClaimsCarry")} office={t("deathClaimsOffice")} timeline={t("deathClaimsTime")} /><Button loading={loading} onClick={() => void startClaims()}>{t("deathSubmitClaims")} <ArrowRight aria-hidden className="size-4" /></Button></StepCard>
  ) : currentStep === 4 ? (
    <StepCard eyebrow={t("deathSharedWorkflow")} title={t("deathConsentTitle")} body={t("deathConsentBody")}><div className="grid gap-2">{["Sunita Sharma", "Arjun Sharma", "Kavita Verma"].map((name, index) => <div className="flex items-center justify-between rounded-[3px] bg-paper-line px-4 py-3 text-sm" key={name}><span>{name}</span><span className={index < 2 ? "text-green-deep" : "text-brick"}>{index < 2 ? t("ready") : t("pending")}</span></div>)}</div><Button loading={loading} onClick={() => void collectHeirConsent()}><ShieldCheck aria-hidden className="size-4" />{t("deathRequestConsent")}</Button></StepCard>
  ) : (
    <StepCard eyebrow={t("deathKeepVisible")} title={t("deathDownstreamTitle")} body={t("deathDownstreamBody")}><div className="grid gap-3 sm:grid-cols-2">{assets.map((asset) => <div className="flex items-center gap-3 rounded-[3px] bg-paper-line p-4" key={asset.id}>{asset.type === "property" ? <Building2 aria-hidden className="size-5 text-brick" /> : <CarFront aria-hidden className="size-5 text-green-deep" />}<div className="min-w-0"><strong className="block text-sm leading-5 [overflow-wrap:anywhere]">{asset.type === "property" ? asset.attrs.authority : asset.type === "vehicle" ? `${asset.attrs.make} ${asset.attrs.model}` : "—"}</strong><span className="text-xs text-ink-mute">{t("deathSuccessionDraft")}</span></div></div>)}</div><Button onClick={finish}>{t("deathCreateDrafts")} <ArrowRight aria-hidden className="size-4" /></Button></StepCard>
  );
