...
import { formatCurrency, formatDate, formatMonthYear, maskIdentifier } from "@/lib/format";
...
function verification(source: Verification["source"]): Verification {
  return { source, state: "verified", asOf: "2026-08-28" };
}

function previousMonth(date: string) {
  const value = new Date(`${date.slice(0, 10)}T00:00:00+05:30`);
...
export function MoneyActionWorkflow({ action }: { action: MoneyAction }) {
  ...
  const refund = getNodeByType(graph, "obl:itr-refund", "obligation");
  ...
  const gstrPeriod = gstr?.attrs.dueDate ? previousMonth(gstr.attrs.dueDate) : undefined;
  ...
  const payPropertyTax = async () => {
    ...
    try {
      ...
      const mutations: GraphMutation[] = [
        ...
        { type: "addNode", node: { id: receiptId, type: "document", attrs: { kind: "payment-receipt", holderName: "Arjun Sharma", numberMasked: maskIdentifier(response.data.receipt), issuedOn: "2026-08-28", authority: "BBMP via Karnataka One", downloaded: true }, verification: verification("Municipal") } },
        { type: "addEdge", edge: { id: "e:arjun-holds-bbmp-tax-receipt", type: "holds", from: personId, to: receiptId, attrs: {}, validFrom: "2026-08-28", status: "active", verification: verification("Municipal") } },
      ];
  ...
  const fileGstr = async () => {
    if (!gstr || !gstrPeriod || !business || gstrComplete) return;
    setLoading(true);
    setError(null);
    try {
      const response = await submitGstr3b({ businessId: "biz:sharma-web", period: gstrPeriod });
      const acknowledgementId = `doc:arjun-gstr3b-${gstrPeriod}-acknowledgement`;
      const mutations: GraphMutation[] = [
        { type: "patchAttrs", nodeId: gstr.id, attrs: { status: "completed", note: `Simulated filing acknowledgement ${response.data.acknowledgement}.` }, verification: { source: "Self", state: "pending", asOf: "2026-08-28", note: "Simulated GSTN acknowledgement." } },
        { type: "addNode", node: { id: acknowledgementId, type: "document", attrs: { kind: "gst-filing-acknowledgement", holderName: business.attrs.name, numberMasked: maskIdentifier(response.data.acknowledgement), issuedOn: "2026-08-28", businessId: business.id, authority: response.authority, downloaded: true }, verification: { source: "Self", state: "pending", asOf: "2026-08-28", note: "Generated by the simulated GSTN filing desk." } } },
        { type: "addEdge", edge: { id: `e:arjun-holds-gstr3b-${gstrPeriod}-acknowledgement`, type: "holds", from: personId, to: acknowledgementId, attrs: {}, validFrom: "2026-08-28", status: "active", verification: { source: "Self", state: "pending", asOf: "2026-08-28" } } },
      ];
      commit({ actorId: personId, labelKey: "eventGstrFiled", procedureId, labelParams: { reference: response.data.acknowledgement }, mutations });
    } catch {
      setError(t("moneyServiceError"));
    } finally {
      setLoading(false);
  ...
  const content = action === "property-tax" ? propertyComplete ? (
    ...
    <StepCard eyebrow="GSTN" title={t("gstrTitle")} body={t("gstrBody")}><div className="flex items-center gap-3 rounded-[8px] bg-indigo-tint p-5"><Landmark aria-hidden className="size-5 text-indigo-deep" /><div><strong className="block text-sm text-ink">{business?.attrs.name ?? "—"}</strong><span className="text-xs text-ink-mute">GSTIN {business?.attrs.gstin ? maskIdentifier(business.attrs.gstin) : "—"} · {gstrPeriod ? formatMonthYear(gstrPeriod, language) : "—"}</span></div></div><label className="flex min-h-12 cursor-pointer items-start gap-3 border-y border-paper-line py-3 text-sm leading-6"><input checked={confirmed} className="mt-1 size-4" onChange={(event) => setConfirmed(event.target.checked)} type="checkbox" /><span>{t("gstrConfirmDeclaration")}</span></label><Button disabled={!confirmed || !gstrPeriod || !business} loading={loading} onClick={() => void fileGstr()}>{t("confirmGstrFiling")} <CheckCircle2 aria-hidden className="size-4" /></Button></StepCard>
  ...
  return <ProcedureShell authority={t("simulatedResponse")} complete={complete} currentStep={complete ? 1 : 0} procedureId={procedureId} showProgress={action === "property-tax" || action === "gstr3b"} steps={steps[language]} title={title}>{error ? <p className="mb-3 text-sm font-bold text-brick">{error}</p> : null}{content}</ProcedureShell>;
}
