...
export function ObligationsWorkflow() {
  ...
  if (personId !== "person:arjun") {
    return <ProcedureShell authority="Bengaluru Traffic Police + Karnataka One" currentStep={0} description={t("obligationsPromise")} procedureId="echallan-payment" steps={steps} title={t("obligationsService")}><StepCard eyebrow="No linked challan" title="Switch to Arjun’s profile" body="This traffic notice belongs to Arjun’s Activa. Citizen will not let another profile pay or change it."><Button onClick={() => switchPersona("person:arjun")} variant="secondary">Switch to Arjun</Button></StepCard></ProcedureShell>;
  }
  ...
  return <ProcedureShell authority="Bengaluru Traffic Police + Karnataka One" complete={complete} currentStep={currentStep} description={t("obligationsPromise")} procedureId="echallan-payment" steps={steps} title={t("obligationsService")}>{error ? <p className="mb-3 rounded-xl bg-brick-tint p-3 text-sm font-semibold text-brick" role="alert">{error}</p> : null}{content}</ProcedureShell>;
}
