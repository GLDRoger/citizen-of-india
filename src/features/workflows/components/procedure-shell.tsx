...
export function ProcedureShell({ title, description, authority, steps, currentStep, children, complete = false, procedureId, outcomeTargetId, showProgress = true }: ProcedureShellProps) {
  const { t } = useI18n();
  const personId = useAuthStore((state) => state.personId);
  const resumed = useWorkflowProgress((state) => Boolean(personId && state.drafts[`${personId}:${procedureId}`]));
  ...
  return (
    <Page className="grid gap-4 lg:gap-7">
      ...
      <header className="grid gap-3 border-t-2 border-ink pt-4 sm:gap-4">
        ...
        {resumed && !complete && currentStep > 0 ? <p className="flex flex-wrap items-baseline gap-x-2 border-l-4 border-saffron pl-3 text-xs leading-5 text-ink-mute"><strong className="text-ink">{t("resumedTitle")}</strong>{t("resumedBody")}</p> : null}
      </header>
...
export function ParticipantStrip({ left, right }: { left: Participant; right: Participant }) {
  ...
  return (
    <>
      ...
      <div className="hidden items-center gap-2 sm:grid sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">{people.map((person, index) => <div className="contents" key={person.name}><article className="flex min-h-18 items-center gap-3 rounded-[3px] border border-paper-line bg-panel px-4"><div className="min-w-0 flex-1"><strong className="block text-sm leading-5 text-ink [overflow-wrap:anywhere]">{person.name}</strong><StatusPill label={person.status} tone={person.tone} /></div></article>{index === 0 ? <ArrowRight aria-hidden className="mx-auto size-4 text-ink-mute" /> : null}</div>)}</div>
    </>
  );
}
