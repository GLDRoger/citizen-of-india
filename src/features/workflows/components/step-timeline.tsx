...
export function StepTimeline({ complete, currentStep, steps }: StepTimelineProps) {
  ...
  return (
    <div className="min-w-0 overflow-hidden rounded-[3px] border border-paper-line bg-panel px-4">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <li className="pr-3 last:pr-0" key={step.id}>
              <div className="flex items-center"><span className={cn("grid size-8 shrink-0 place-items-center rounded-[2px] border text-xs font-black", done ? "border-green-deep bg-green-deep text-paper" : active ? "border-indigo-deep bg-indigo-deep text-paper" : "border-paper-line bg-paper text-ink-mute")}>{done ? <Check aria-hidden className="size-4" /> : index + 1}</span>{index < steps.length - 1 ? <span aria-hidden className={cn("h-px min-w-8 flex-1", done ? "bg-green-deep" : active ? "bg-indigo" : "bg-paper-line")} /> : null}</div>
