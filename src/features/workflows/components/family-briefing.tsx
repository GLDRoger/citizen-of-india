  ...
  return (
    <FilePanel className="grid gap-8 sm:p-8" label={t("familyBriefing")}>
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <article className="grid content-start gap-4 rounded-[3px] border border-green-deep/25 bg-green-tint p-5"><StatusPill label={t("eligible")} tone="success" /><div><h3 className="font-display text-2xl font-bold text-ink">{t("familyPensionEligible")}</h3><p className="mt-1 text-sm font-bold text-green-deep">≈ {t("monthlyAmount", { amount: "₹4,100" })}</p></div><ContrastLine>{t("contrastFamilyPension")}</ContrastLine>{familyPension ? <ReasonList reasons={familyPension.passedReasons} /> : null}</article>
        <article className="grid content-start gap-4 rounded-[3px] border border-brick/20 bg-brick-tint p-5"><StatusPill label={t("missingEvidence")} tone="warning" /><div><h3 className="font-display text-2xl font-bold text-ink">{t("widowPensionBlocked")}</h3><p className="mt-1 text-xs leading-5 text-brick">{t("blockedByIncomeDeclaration")}</p></div>{widowPension ? <ReasonList reasons={widowPension.passedReasons} /> : null}</article>
      </section>
