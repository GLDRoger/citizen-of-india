...
import { delegatedScopes } from "@/features/graph/delegation";
...
function useNavigationItems(): NavItem[] {
  ...
  if (!actorId || !personId) return navItems;
  const items: NavItem[] = [{ href: "/home", label: "sharedRecordTitle", icon: House }];
  if (delegatedScopes(graph, actorId, personId).includes("documents")) items.push({ href: "/documents", label: "documents", icon: FileText });
  return items;
}
...
function MobileNavigation() {
  ...
  const { t } = useI18n();
  return (
    <nav aria-label={t("primaryNavigation")} style={{ gridTemplateColumns: `repeat(${items.length}, minmax(0, 1fr))` }} className="fixed inset-x-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-50 grid rounded-[3px] border border-paper/20 bg-indigo-deep p-1.5 text-paper shadow-[0_4px_12px_rgba(19,28,75,0.18)] min-[900px]:hidden">
      {items.map(({ href, icon: Icon, label }) => {
        const active = isActive(pathname, href);
        return <Link aria-current={active ? "page" : undefined} className={cn("flex min-h-16 min-w-0 flex-col items-center justify-center gap-1 rounded-[2px] px-1 text-center text-xs font-bold leading-4 transition-colors", active ? "bg-saffron text-ink" : "text-paper/72 hover:bg-paper/10 hover:text-paper")} href={href} key={href}><Icon aria-hidden className="size-4.5 shrink-0" /><span className="max-w-full break-normal text-[0.6875rem] min-[380px]:text-xs">{t(label)}</span></Link>;
      })}
    </nav>
  );
}
...
function AccountMenu() {
  ...
  const delegation = personId && !actorId ? getDelegationFor(graph, personId) : undefined;
  const delegator = delegation ? getPerson(graph, delegation.attrs.delegatorId) : undefined;
  const subject = actorId && personId ? getPerson(graph, personId) : undefined;
  ...
  return (
    <details className="group relative" ref={detailsRef}>
      ...
      <div className="absolute right-0 top-12 z-50 grid max-h-[calc(100dvh-4.5rem)] w-[min(22rem,calc(100vw-2rem))] gap-2 overflow-y-auto rounded-[3px] border border-paper-line bg-panel p-3 text-ink shadow-[0_6px_18px_rgba(19,28,75,0.14)]">
        ...
        {subject ? <button className="flex min-h-11 items-center gap-3 rounded-[2px] border border-saffron bg-saffron/15 px-3 text-left text-sm font-bold text-ink hover:bg-saffron/30" onClick={() => { stopActing(); close(); router.push("/home"); }} type="button"><UsersRound aria-hidden className="size-4 text-brick" />{t("stopActing", { name: subject.attrs.name.split(" ")[0] })}</button> : null}
        <div className="grid gap-1 border-t border-paper-line pt-3"><p className="eyebrow">{t("switchPerson")}</p>{seedLogins.map((login) => {
          ...
          return <button className={cn("flex min-h-11 items-center gap-3 rounded-[2px] px-3 text-left text-sm transition-colors", active ? "bg-indigo-tint font-bold text-indigo-deep" : "hover:bg-paper")} key={login.personId} onClick={() => { switchPersona(login.personId); close(); router.push("/home"); }} type="button"><span className="grid size-7 place-items-center rounded-[2px] bg-indigo-deep text-xs font-bold text-paper">{getInitials(candidate.attrs.name)}</span>{candidate.attrs.name}</button>;
        })}</div>
