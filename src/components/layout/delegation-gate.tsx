  ...
  return (
    <Page className="grid gap-7">
      ...
      {permission ? <>
        ...
        {sharedAlerts.length ? <section className="grid gap-4"><h2 className="font-display text-2xl font-semibold">{t("familySharedAlertsTitle")}</h2><p className="text-sm leading-6 text-ink-mute">{t("familySharedAlertsIntro")}</p><div className="border-y border-paper-line">{sharedAlerts.map((alert) => { const relationshipKey = getRelationshipMessageKey(alert.relationship); const relationship = relationshipKey ? t(relationshipKey) : alert.relationship; const title = localizeNodeTitle(language, alert.recordId, alert.title); return <article className="grid gap-1 border-b border-paper-line py-4 last:border-b-0" key={alert.id}><p className="eyebrow text-indigo-deep">{t("familySharedUpdateLabel")}</p><h3 className="font-display text-xl font-semibold leading-tight text-ink">{t("familyAlertTitle", { relationship, name: alert.ownerName, title })}</h3><p className="text-xs text-ink-mute">{alert.meta}{alert.dueDate ? ` · ${formatDate(alert.dueDate, language)}` : ""}</p></article>; })}</div></section> : null}
        {!scopes.includes("documents") && !properties.length && !sharedAlerts.length ? <p className="border-y border-paper-line py-5 text-sm text-ink-mute">{t("familyNoSharedRecords")}</p> : null}
